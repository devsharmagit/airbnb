...
if(!selected.includes(name)){
  ...
  return (
    ...
    {perksArr.map(({text, Svg, perk})=>{
      return <div key={text} onClick={()=> handleClick(perk)} className={`flex items-center justify-center gap-2 border border-black ${selected?.includes(perk) && "bg-primary text-white"} rounded-xl min-h-[50px] ${forDisplay && !allowedPerks?.includes(perk) ? "hidden" : "" }`}>
      <Svg />
       <p>{text}</p>
     </div>
    })}
