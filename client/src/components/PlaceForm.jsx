...
function PlaceForm({ type, placeId, place }) {
  ...
  return (
    <>
    <LoadingModal isOpen={loading} text={"Loading... I am using free tier of cloudnary 🥲. It will take time please wait."} />
    <form
      onSubmit={handleSubmit(handleFormSubmit)}
      className="max-w-screen-xl m-auto px-4 pb-4"
    >
      <div className="mt-4">
      ...
      <div className="mt-4">
        ...
        <p className="text-gray-500 font-medium text-sm pl-2 mont">
          {error && error}
        </p>
        <PhotosUploader
          addedPhoto={photos}
          setAddedPhoto={setPhotos}
          error={error}
        />
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 mb-8 md:grid-cols-3 lg:grid-cols-4  gap-2">
      ...
      <div className="mt-4">
        ...
        <TextArea
          errorMsg={errors?.extraInfo?.message}
          {...register("extraInfo")}
        />
      </div>
      <div className="mt-4">
        {inputCombo(
          "Location",
          "Enter Latitude and Longitutde coordinates of the Place"
        )}
...
<div className="flex gap-5 justify-center">
      <Button text={"Cancel"}  className={"bg-gray-600 m-0"} onClick={()=> navigate("/")}/>
      <Button text={"Save"} className={"m-0"} />
...
function inputPara(para) {
  return <Paragrapgh className={"leading-tight text-sm"} text={para} />;
}
